...
milestones (scaffolding, then a commit per module as its tests go green, then
a final docs/wiring commit), never as one giant commit.
...
- `DESIGN.md`: the two-layer leakage argument written down so the reasoning
  survives losing any prompt or conversation.
- `PLAN.md`: this file.
...
- `TemporalGuard(as_of, policy)`: filters a list of records.
  - `strict` drops violations.
  - `warn` keeps them but flags them.
...
- A wrapper (decorator or higher-order function) taking **any** Python
...
- Native tool-calling where the model supports it, text-parsing ReAct fallback
  where it doesn't.
...
- One CLI command runs an end-to-end scenario (agent run, leakage probe, claim
  classification) and emits both a human-readable report and a machine-readable
